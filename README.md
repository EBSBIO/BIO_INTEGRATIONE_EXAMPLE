...
Эталонная реализация для 3х АПИ: приемка уведомлений о создании УЗ(idp-notifier), приемка уведомлений о мэтчинге УЗ(
matcher-notifier), приемник шаблонов и деактивации(bioskud-router). Проект принимает запрос от idp-notifier,
matcher-notifier и bioskud-router, логирует полученные данные и отвечает.
...
Обработка уведомлений о мэтчинге УЗ:
    ...
    /api/matcher_notifier

Обработка запроса деактивации шаблонов:

    /api/templates/deactivation
...
Заголовки запроса: Authorization: Bearer {токен от контрагента}, Content-Type: application/json

**Примечание**: для idp ESIA токен формируется в формате JWT

**Тело запроса**:
| Наименование параметра | Тип данных | Обязательность | Описание |
|------------------------|------------|----------------|------------------------------------------| | user_id | String |
нет | Идентификатор УЗ пользователя IDP | | stu | String | да | Статус результата регистрации пользователя в ЕБС.
Принимает значение: " A" - пользователь успешно зарегистрирован "B" - пользователь заблокирован "D" - пользователь
деактивирован "F " - неуспешная попытка регистрации пользователя
...
| Наименование параметра | Тип данных | Обязательность | Описание |
|------------------------|------------|----------------|------------------------------------------| | description |
String | нет | Текст ошибки | | request_id | String | да | идентификатор запроса | stu | String | да | Статус результата
регистрации пользователя в ЕБС. Принимает значение: " MA" - пользователь успешно зарегистрирован "MF " - не успешная
попытка регистрации пользователя | user_id | String | да |
...
где Cookie - идентификатор запроса (транзакции), который служит для сопоставления с ранее отправленным запросом на
регистрацию данных.
...
[Коллекция запросов для Postman](docs/Эталонная реализация.postman_collection.json)
